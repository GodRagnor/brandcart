...
Supports: Twilio (SMS), SMTP (Email), Custom providers
"""
...
def _normalize_e164_phone(value: str, *, default_country_code: str = "+91") -> str:
    ...
    return f"{default_country_code}{digits}" if digits else ""
...
async def send_otp_sms(phone: str, otp: str) -> bool:
    ...
    Args:
        phone: Phone number in local or E.164 format
    ...
    if SMS_PROVIDER == "none":
        logger.warning("SMS_PROVIDER set to 'none', OTP not sent to %s", phone)
        return False
...
async def _send_via_twilio(phone: str, otp: str) -> bool:
    ...
    try:
        ...
        formatted_phone = _normalize_e164_phone(phone)
        from_phone = _normalize_e164_phone(TWILIO_FROM_PHONE, default_country_code="+")
        if not formatted_phone or not from_phone:
            logger.error("Twilio phone configuration is invalid")
            return False
        ...
        logger.info("OTP sent via Twilio to %s: %s", formatted_phone, msg.sid)
        return True
    except Exception as error:
        logger.error("Failed to send OTP via Twilio: %s", error)
        return False
...
async def send_otp_email(email: str, otp: str, phone: str = "") -> bool:
    ...
    if EMAIL_PROVIDER == "none":
        logger.warning("EMAIL_PROVIDER set to 'none', OTP email not sent to %s", email)
        return False

    logger.info("Email OTP provider '%s' not configured, skipping", EMAIL_PROVIDER)
    return False
...
async def _send_via_smtp(email: str, otp: str) -> bool:
    ...
    try:
        ...
        logger.error("Failed to send OTP email: %s", error)
        return False
...
async def notify_otp(phone: str, email: Optional[str] = None, otp: str = "") -> dict:
    ...
    if OTP_DEV_MODE:
        logger.warning("OTP_DEV_MODE enabled - OTP for %s is: %s", phone, otp)
